...
class Obstacle(pg.sprite.Sprite):
    def __init__(self, pos):
        super().__init__()
        self.image = pg.Surface((30, 30)).convert()
        self.image.fill((255, 255, 255))  # White color for the obstacle
...
def main():
    ...
    obstacles = pg.sprite.Group()  # Group for obstacles
    ...
    while True:
        for e in pg.event.get():
            if e.type == pg.QUIT or e.type == pg.KEYDOWN and (e.key == pg.K_ESCAPE or e.key == pg.K_q or e.key==pg.K_SPACE):
                return
            elif e.type == pg.MOUSEBUTTONDOWN and e.button == 1:  # Add boid on left mouse button clicked
                pos = pg.mouse.get_pos()
                new_boid = Boid(boidTracker, screen, FISH)
                new_boid.rect.center = pos
                nBoids.add(new_boid)
            elif e.type == pg.MOUSEBUTTONDOWN and e.button == 3:  # Add obstacle on right mouse button clicked
                pos = pg.mouse.get_pos()
